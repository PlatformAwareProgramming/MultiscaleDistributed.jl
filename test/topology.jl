...
function launch(manager::TopoTestManager, params::Dict, launched::Array, c::Condition)
    ...
    cmd = `$exename $exeflags --bind-to $(MultiscaleCluster.LPROC.bind_addr) --worker`
    cmd = pipeline(detach(setenv(cmd, dir=dir)))
    for i in 1:manager.np
        io = open(cmd, "r+")
        MultiscaleCluster.write_cookie(io)
...
function def_count_conn()
    @everywhere function count_connected_workers()
        count(x -> isa(x, MultiscaleCluster.Worker) && isdefined(x, :r_stream) && isopen(x.r_stream),
                MultiscaleCluster.PGRP().workers)
    end
end
